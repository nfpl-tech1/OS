...
export default function BranchesPage() {
  ...
  async function handleExpand(branchId: string) {
    if (expandedId === branchId) { setExpandedId(null); return; }
    setExpandedId(branchId);
    if (defaultApps[branchId]) return;
    setDefaultAppsLoading(prev => ({ ...prev, [branchId]: true }));
    try {
      const apps = await getBranchDefaultApps(branchId);
      setDefaultApps(prev => ({ ...prev, [branchId]: apps }));
    } finally {
      setDefaultAppsLoading(prev => ({ ...prev, [branchId]: false }));
    }
  }

  async function handleToggleApp(branchId: string, app: App) {
    const current = defaultApps[branchId] ?? [];
    const isAssigned = current.some(a => a.id === app.id);
    const key = `${branchId}-${app.id}`;
    setTogglingApp(key);
    try {
      if (isAssigned) {
        await removeBranchDefaultApp(branchId, app.id);
        setDefaultApps(prev => ({
          ...prev,
          [branchId]: prev[branchId].filter(a => a.id !== app.id),
        }));
      } else {
        await addBranchDefaultApp(branchId, app.id);
        setDefaultApps(prev => ({
          ...prev,
          [branchId]: [...(prev[branchId] ?? []), { id: app.id, slug: app.slug, name: app.name }],
        }));
      }
    } catch (err: any) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setAddLoading(true);
    try {
      const branch = await createBranch(newName.trim());
      setBranches(prev => [...prev, branch]);
      setNewName('');
      setAdding(false);
    } catch (err: any) {
      setAddError(err?.response?.data?.message ?? 'Failed to create branch');
    } finally {
      setAddLoading(false);
    }
  }
  ...
  async function handleUpdate(id: string) {
    ...
    setEditLoading(true);
    try {
      const updated = await updateBranch(id, editName.trim());
      setBranches(prev => prev.map(d => (d.id === id ? { ...d, name: updated.name } : d)));
      setEditingId(null);
    } catch (err: any) {
          ...
          <form onSubmit={handleAdd} className="bg-primary/5 rounded-3xl p-6 border border-primary/10 flex flex-col md:flex-row items-start md:items-center gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="flex-1 w-full space-y-1">
              <input
                ...
                placeholder="Branch name (e.g. Logistics Ops)"
                required
        ...
        <div className="space-y-3">
          {branches.length === 0 && !adding && (
            <div className="bg-white rounded-3xl border border-dashed border-slate-200 py-16 text-center">
              ...
              <p className="text-slate-400 text-xs mt-1 font-medium">Provision branches to automate member onboarding.</p>
            </div>
          )}

          {branches.map((branch) => {
            const isExpanded = expandedId === branch.id;
            const assigned = defaultApps[branch.id] ?? [];
            ...
            return (
              <div key={branch.id} className={cn(
                  ...
                  {editingId === branch.id ? (
                    <div className="flex-1 flex flex-col md:flex-row items-start md:items-center gap-3 w-full">
                      ...
                      <div className="flex gap-2 w-full md:w-auto">
                        <Button onClick={() => handleUpdate(branch.id)} disabled={editLoading} size="sm" className="flex-1 md:flex-none h-10 px-6 bg-primary text-white font-bold rounded-xl">
                    ...
                    <>
                      <button onClick={() => handleExpand(branch.id)} className="flex-1 flex items-center gap-4 text-left min-w-0">
                        <div className={cn(
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{branch.slug}</p>
                        </div>
                      ...
                      <div className="flex items-center gap-1">
                        {deletingId === branch.id ? (
                          <div className="flex items-center gap-2 animate-in fade-in slide-in-from-right-2">
                             ...
                             <Button onClick={() => handleDelete(branch.id)} disabled={deleteLoading} size="sm" className="h-8 bg-red-500 hover:bg-red-600 text-white font-bold rounded-lg px-3">
                               Confirm
                             </Button>
                          ...
                          <>
                            <Button onClick={(e) => { e.stopPropagation(); startEdit(branch); }} variant="ghost" size="sm" className="h-8 w-8 p-0 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-lg border border-slate-100 transition-all shadow-sm">
                              <Icons.Edit size={14} />
                            </Button>
                            <Button onClick={(e) => { e.stopPropagation(); setDeletingId(branch.id); }} variant="ghost" size="sm" className="h-8 w-8 p-0 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg border border-slate-100 transition-all shadow-sm">
                              <Icons.Trash size={14} />
                            </Button>
                            <div className={cn(
                ...
                {isExpanded && editingId !== branch.id && (
                  <div className="px-6 pb-6 pt-2 border-t border-slate-50 animate-in fade-in zoom-in-95 duration-300">
                    ...
                    {defaultAppsLoading[branch.id] ? (
                      ...
                      <div className="flex flex-wrap gap-2">
                        {allApps.map(app => {
                          ...
                          const isToggling = togglingApp === `${branch.id}-${app.id}`;
                          
                          return (
                            <button
                              key={app.id}
                              onClick={() => handleToggleApp(branch.id, app)}
                              disabled={isToggling}
                              className={cn(
                    ...
                    <div className="mt-6 flex items-start gap-3 bg-slate-50/50 rounded-2xl p-4 border border-slate-100">
                      ...
                      <p className="text-[11px] text-slate-500 font-medium leading-relaxed">
                        Automatic Onboarding: New accounts assigned to the <strong className="text-slate-900">{branch.name}</strong> unit will inherit access to all bridged modules above by default.
                      </p>
                    </div>
                  </div>
                )}
